...
class VisionGenerateRequest(BaseModel):
    ...
    model_name: Optional[str] = Field(
        ...
        description="Checkpoint name (e.g. dreamshaper_8.safetensors)",
    )
...
class VisionWorkflowRequest(BaseModel):
    workflow_yaml: str = Field(
        ..., description="Path to workflow YAML file (e.g., hires_fix.sd15.dreamshaper)"
    )
    config_yaml: str = Field(
        ..., description="Path to data config YAML (e.g., cyberpunk_streetscape)"
    )
    seed: Optional[int] = Field(None, description="Random seed")


class VisionWorkflowResponse(BaseModel):
    status: str
    file_path: Optional[str] = None
    message: Optional[str] = None
    metadata: Optional[Dict[str, Any]] = None
