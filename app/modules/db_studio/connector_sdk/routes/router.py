...
from common_lib.modules.db_studio.connector_sdk import (
    ConnectorSDKService,
    ConnectorCreate, ConnectorUpdate, ConnectorOut,
    DriverCreate, DriverOut,
    PluginRegister, PluginOut,
    CapabilityCreate, CapabilityOut,
    CertificationCreate, CertificationOut,
)

router = APIRouter(prefix="/api/v1/connector-sdk", tags=["Connector SDK"])
svc = ConnectorSDKService()


@router.post("/connectors", response_model=ConnectorOut)
def register_connector(req: ConnectorCreate): return svc.register_connector(req)

@router.get("/connectors", response_model=list[ConnectorOut])
def list_connectors(connector_type: str = None, is_enabled: bool = None,
...
def delete_connector(connector_id: str):
    if not svc.delete_connector(connector_id): raise HTTPException(404)
    return {"ok": True}

@router.post("/drivers", response_model=DriverOut)
def register_driver(req: DriverCreate): return svc.register_driver(req)
...
def delete_driver(driver_id: str):
    if not svc.delete_driver(driver_id): raise HTTPException(404)
    return {"ok": True}

@router.post("/plugins", response_model=PluginOut)
def register_plugin(req: PluginRegister): return svc.register_plugin(req)
...
def delete_plugin(plugin_id: str):
    if not svc.delete_plugin(plugin_id): raise HTTPException(404)
    return {"ok": True}

@router.post("/capabilities", response_model=CapabilityOut)
def register_capability(req: CapabilityCreate): return svc.register_capability(req)
...
def delete_capability(capability_id: str):
    if not svc.delete_capability(capability_id): raise HTTPException(404)
    return {"ok": True}
...
@router.post("/certifications/{certification_id}/certify", response_model=CertificationOut)
