...
Thin wrappers delegating business logic to common_lib services.
"""
...
_limiter = get_log_viewer_limiter()
...
async def get_log_levels_endpoint():
    return get_log_levels()


@router.post("/logging/levels")
async def set_log_level_endpoint(payload: Dict[str, Any] = Body(...)):
...
async def remove_team_member(user_id: int, session: Session = Depends(get_session)):
    try:
        deactivate_member(session, user_id)
        return {"status": "success", "message": f"Member deactivated"}
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
