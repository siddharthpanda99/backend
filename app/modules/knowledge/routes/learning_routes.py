...
from app.modules.knowledge.routes import ConfigUpdateRequest
from app.modules.knowledge.routes.learning import _get_learning_instance
...
async def update_reasoner_config(
    ...
    try:
        ...
        config = reasoner.update_config(
            short_query_threshold=int(short_query_threshold)
            if short_query_threshold is not None
            else None,
            enable_hyde_suggestion=bool(enable_hyde_suggestion)
            if enable_hyde_suggestion is not None
            else None,
...
async def update_belief_config(
    ...
    try:
        ...
        config = reviser.update_config(
            ...
            constant_learning_rate=float(constant_learning_rate)
...
async def update_conflict_resolver_config(
    ...
    try:
        ...
        config = resolver.update_config(
            ...
            min_confidence_gap=float(min_confidence_gap)
            if min_confidence_gap is not None
            else None,
            min_source_trust_gap=float(min_source_trust_gap)
            if min_source_trust_gap is not None
            else None,
            enable_auto_resolution=bool(enable_auto_resolution)
            if enable_auto_resolution is not None
            else None,
        )
        return {
            "success": True,
...
async def update_evolution_branching_config(
    ...
    try:
        ...
        config = instance.update_config(
            enable_branching=bool(enable_branching)
            ...
            specialization_threshold=float(specialization_threshold)
            if specialization_threshold is not None
            else None,
        )
        return {
            "success": True,
...
async def update_knowledge_pruner_config(
    ...
    try:
        ...
        config = instance.update_config(
            ...
            enable_user_review=bool(enable_user_review)
            if enable_user_review is not None
            else None,
            auto_prune_threshold=float(auto_prune_threshold)
            if auto_prune_threshold is not None
            else None,
        )
        return {
            "success": True,
