...
from common_lib.modules.knowledge_engine.learning.evolution_orchestrator import (
...
# Lazy-initialized evolution orchestrator
_evolution_orchestrator: Optional[EvolutionOrchestrator] = None


def _get_orchestrator() -> EvolutionOrchestrator:
    ...
    if _evolution_orchestrator is None:
        _evolution_orchestrator = EvolutionOrchestrator(
            ...
            adaptive_strategy=_get_learning_instance("adaptive_strategy"),
...
def _get_learning_instance(name: str) -> Any:
    ...
    if name not in _learning_instances:
        if name == "quality_log":
            ...
            # Wire up DB persistence callback using common_lib model directly
            def _persist_assessment(report):
                try:
                    from sqlmodel import create_engine, Session as SASession
                    import os
...
async def record_quality_log(
    ...
    try:
        ...
        # Auto-evolution orchestration via EvolutionOrchestrator
        try:
...
async def get_heatmap_document_stats(
    ...
    """
    try:
        # Enrich with quality log data if available
        ...
        data = AnalyticsService.document_stats(
            session=session,
            limit=limit,
            source_query_count=source_query_count or None,
        )
