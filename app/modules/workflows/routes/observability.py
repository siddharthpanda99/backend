...
def get_execution_summary(trace_id: str, session: Session = Depends(get_session)):
    ...
    for event in events:
        if event.node_output and isinstance(event.node_output, dict):
            ...
            elif "width" in event.node_output and "height" in event.node_output: # Fallback
                compute["resolution"] = f"{event.node_output['width']}x{event.node_output['height']}"
    ...
    if usage["total_tokens"] > 0:
        ...
        est_cost = (usage["prompt_tokens"] / 1000 * 0.01) + (usage["completion_tokens"] / 1000 * 0.03)
    else:
    ...
    throughput = {
        "tokens_per_sec": round(usage["total_tokens"] / duration_sec, 2) if usage["total_tokens"] > 0 else 0,
        "nodes_per_sec": round(len(events) / duration_sec, 2),
        "avg_node_latency_ms": round(execution.duration_ms / len(events), 2) if events else 0
    }
    ...
    data["intelligence"] = {
        ...
        "benchmarks": {
            ...
            "diff_percent": ((execution.duration_ms - avg_duration) / avg_duration * 100) if avg_duration and execution.duration_ms else 0
        },
        "usage": {
            **usage,
...
def get_observability_stats(session: Session = Depends(get_session)):
    ...
    status_statement = select(WorkflowExecution.status, func.count(WorkflowExecution.trace_id)).group_by(WorkflowExecution.status)
    ...
    history_statement = select(WorkflowExecution.started_at, WorkflowExecution.status).where(WorkflowExecution.started_at >= one_day_ago)
