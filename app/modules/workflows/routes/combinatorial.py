...
import asyncio, datetime, itertools, json, logging, subprocess, time, traceback, uuid
...
class CommonConfig(BaseModel):
    steps: List[int] = [20]
    cfg: List[float] = [7.5]
    sampler: List[str] = ["euler"]
    scheduler: List[str] = ["karras"]
    sizes: List[str] = ["768x768"]
    checkpoint_name: List[str] = ["dreamshaper_8.safetensors"]
    model_type: List[str] = ["sd15"]
    seed: List[int] = [-1]
...
class OutputConfig(BaseModel):
    baseDirectory: str = "generated_content/combinatorial"
    filenameFormat: str = "{workflow_id}_{prompt_hash}_{timestamp}"
...
class CombinatorialGenerateRequest(BaseModel):
    workflows: List[str]
...
def _compute_cartesian(common: CommonConfig) -> List[Dict[str, Any]]:
    sizes_parsed = []
    for s in common.sizes:
        if "x" in s:
            ...
            try:
                ...
                sizes_parsed.append((w, h))
            except ValueError:
                sizes_parsed.append((768, 768))
        else:
            sizes_parsed.append((768, 768))
    ...
    fields = {
        ...
        "checkpoint_name": common.checkpoint_name,
        "model_type": common.model_type,
        "seed": common.seed,
    }
    keys = list(fields.keys())
    value_lists = [fields[k] for k in keys]

    combos = []
    for w, h in sizes_parsed:
...
async def _run_one(
    ...
    async with sem:
        logger.info(
            f"[Combinatorial] [{idx + 1}/{total}] {wf_id} | s={combo['sampler']} st={combo['steps']} cfg={combo['cfg']} {combo['width']}×{combo['height']} seed={combo['seed']}"
        )
            ...
            if runner:
                ...
                try:
                    result = await _run_subprocess(
                        cmd, str(CLI_DIR), execution_cfg.timeoutMinutes * 60
                    )
            ...
            for f in sorted(GENERATED_DIR.rglob("*.png")):
                ...
                if age < 300:
                    return {
                        "workflowId": wf_id,
                        "filename": f.name,
                        "path": str(f.relative_to(GENERATED_DIR)),
                        "size": f.stat().st_size,
                        "width": combo["width"],
                        "height": combo["height"],
                        "status": "completed",
...
async def combinatorial_history():
    """List all past combinatorial executions (summary only)."""
    entries = []
        ...
        entries.append(
            {
                ...
                "workflowCount": len(record["config"]["workflows"]),
                "promptCount": len(record["config"]["prompts"]),
            }
        )
...
async def combinatorial_history_by_id(execution_id: str):
    """Get full details for a past combinatorial execution."""
...
async def combinatorial_generate(req: CombinatorialGenerateRequest):
    ...
    param_combos = _compute_cartesian(req.common)
    ...
    if total > WARN_THRESHOLD:
        logger.warning(
            f"[Combinatorial] {execution_id}: LARGE GENERATION — {total} total images (threshold={WARN_THRESHOLD})"
        )

    logger.info(
        f"[Combinatorial] {execution_id}: {len(req.workflows)}w × {len(prompts)}p × {len(param_combos)}c = {total} total, maxConcurrent={req.execution.maxConcurrent}"
    )
    ...
    for wi, wf_id in enumerate(req.workflows):
        for pi, pc in enumerate(prompts):
            for ci, combo in enumerate(param_combos):
                ...
                overrides = {
                    "prompt": pc.prompt,
                    "negative_prompt": pc.negativePrompt,
                    **combo,
                }
                idx = (
                    wi * len(prompts) * len(param_combos) + pi * len(param_combos) + ci
                )
                tasks.append(
                    _run_one(
                        sem,
                        runner if has_runner else None,
                        wf_id,
                        output_subdir,
                        overrides,
                        combo,
    ...
    # Persist to in-memory history store
    _execution_store[execution_id] = {
        ...
        "total": total,
        "warnThreshold": total > WARN_THRESHOLD,
        "summary": summary,
