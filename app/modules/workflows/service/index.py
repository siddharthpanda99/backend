...
class WorkflowService:
    ...
    async def run_graph_stream(
        ...
        logger.info(
            f"[WorkflowService] INITIATING Graph Stream | Nodes: {len(nodes)} | Edges: {len(edges)}"
        )
        ...
        class QueueTracer(EventTracer):
            ...
            def emit(self, event):
                try:
                    data = event.to_dict()
                    logger.info(
                        f"[QueueTracer] EMIT: {data.get('event_type')} (state_id={data.get('state_id')})"
                    )

                    # Extract error for debugging
                    error_msg = data.get("error") or (data.get("metadata") or {}).get(
                        "error"
                    )
                    if error_msg:
                        import sys
        ...
        for n in nodes:
            ...
            state = State(
                ...
                timeout_seconds=n.get(
                    "timeout_seconds", props.get("timeout_seconds", 300)
                ),
            )
        ...
        for sid in base_sorted:
            ...
            if "load_image" in t_id:
                loaders.append(sid)
            elif "save_profile" in t_id:
                savers.append(sid)
            else:
                middle.append(sid)
        ...
        logger.info(
            f"[WorkflowService] ENFORCED SEQUENCE: {' -> '.join(final_sequence)}"
        )
        ...
        graph = Graph(
            id=graph_id,
            name="Character DNA Pipeline",
            start_state_id=final_sequence[0] if final_sequence else "unknown",
        )
        ...
        for i in range(len(final_sequence) - 1):
            ...
            if curr_id in state_map and next_id in state_map:
                state_map[curr_id].transitions.append(
                    Transition(
                        to_state_id=next_id, description="Linear sequence enforced"
                    )
                )
        ...
        for e in edges:
            ...
            if parent_state and target_state:
                ...
                target_port = (
                    str(target_port_raw).lower() if target_port_raw else "input"
                )
                source_port = (
                    str(source_port_raw).lower() if source_port_raw else "output"
                )
                ...
                if "face_analysis" in source_type:
                    if target_port in ["tags", "character_notes"]:
                        output_key = "dna_tags"
                    elif target_port == "face_embedding":
                        output_key = "face_embedding"
                    elif target_port == "body_measurements":
                        output_key = "measurements"
                    elif target_port == "color_palette":
