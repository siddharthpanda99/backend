...
import { readJsonFileAsync, writeJsonFileAsync } from "src/utils/asyncReadWrite";
...
export const requestBalSheet = async (req: Request, res: Response) => {
    try {
        const provider = req.params.provider;
        const companySheet = await readJsonFileAsync(`src/fake-repository/loan/Bal_sheet_${provider}.json`);
        res.send({ data: companySheet, message: `Got the company sheet from: ${provider}` });
    } catch (error) {
        res.status(500).send({ error: 'Internal Server Error' });
    }
...
export const initiateLoanApp = async (req: Request, res: Response) => {
    try {
        ...
        const fileBuffer = await readJsonFileAsync('src/fake-repository/LoanApplications.json').toString();
        ...
        await writeJsonFileAsync('src/fake-repository/LoanApplications.json', JSON.stringify(LoansList, null, 2));
